...
def preprocess(sentence):
    ...
    matching_string=re.compile('[a-zA-Z]')

    word_tokens=nltk.word_tokenize(sentence)

    return [i.lower() for i in word_tokens if matching_string.match(i)]
...
def np_chunk(tree):
    ...
    for tree in parented_tree.subtrees():
        ...
        if tree.label() == "N":
            noun_phrase_chunks.append(tree.parent())
