...
class CrosswordCreator():
    ...
    def save(self, assignment, filename):
        ...
        #font = ImageFont.truetype("assets/fonts/OpenSans-Regular.ttf", 80 ,encoding="unic")
        #[ ImageFont.truetype("assets/fonts/OpenSans-Regular.ttf", 80) for _ in range(1000)]
    ...
    def revise(self, x, y):
        ...
        if self.crossword.overlaps[x, y] is None:
            ...
            for word_x in self.domains[x]:
                overlapping_possible = False
                for word_y in self.domains[y]:
                    if word_x[a] == word_y[b]:
                        overlapping_possible = True
                if not overlapping_possible:
                    WordsToRemove.append(word_x)
            if not WordsToRemove:
                return False
            ...
            if i in assignment:
                ...
                for j in neighbor:
                    if i in self.domains[j]:
                        cnt += 1
                domain_dict[i] = cnt
        return sorted(domain_dict, key=lambda k: domain_dict[k])
